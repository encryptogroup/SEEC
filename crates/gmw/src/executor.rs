    ...
    pub async fn execute(
        ...
        for layer in CircuitLayerIter::new(self.circuit) {
            for (gate, sc_gate_id) in layer.non_interactive_iter() {
                let output = if let Some(BaseGate::Input(_)) = gate.as_base_gate() {
                    assert_eq!(
                    ...
                    let inp = inputs.get(inp_idx);
                    let inp2 = self.gate_outputs[0].get(inp_idx).clone();
                    trace!(
                        "party {}, gate {}\n{:?}\n{:?}",
                        self.party_id,
                        inp_idx,
            ...
            sender.send(msg.clone()).await.ok().unwrap();
            debug!("Sending interactive gates layer");
            let response = receiver.recv().await.ok().unwrap().unwrap();
            ...
            layer
                ...
                .for_each(|((_, id), out)| {
                    self.set_gate_output(id, out.clone());
    ...
    fn gate_inputs(
        ...
        self.circuit.parent_gates(id).map(move |parent_id| {
            ...
            assert!(
                ...
                "parent {parent_id} of {id} not set",
            );
