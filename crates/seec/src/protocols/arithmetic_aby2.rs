...
use crate::protocols::aby2::{DeltaSharing, ShareType};
use crate::protocols::arithmetic_gmw::ArithmeticGmw;
use crate::protocols::{
    arithmetic_gmw, FunctionDependentSetup, Gate, Protocol, Ring, ScalarDim, SetupStorage,
    ShareStorage,
};
    ...
    fn setup_gate_outputs<Idx: GateIdx>(
        ...
        let storage: Vec<_> = circuit
            ...
            .map(|(gate_count, simd_size)| {
                assert_eq!(
                    None, simd_size,
                    "SIMD not supported for arithmetic ABY2 protocol"
                );
    ...
    distributions::Standard: Distribution<R>,
{
    ...
    pub fn from_gmw_gate(g: arithmetic_gmw::ArithmeticGate<R>) -> Self {
        match g {
            arithmetic_gmw::ArithmeticGate::Base(base_gate) => Self::Base(base_gate),
            arithmetic_gmw::ArithmeticGate::Mul => Self::Mul,
            arithmetic_gmw::ArithmeticGate::Add => Self::Add,
            arithmetic_gmw::ArithmeticGate::Sub => Self::Sub,
        }
    }
}

#[async_trait]
impl<MtpErr, Mtp, Idx, R> FunctionDependentSetup<ArithmeticAby2<R>, Idx>
    for AbySetupProvider<Mtp, R>
where
    ...
    distributions::Standard: Distribution<R>,
{
    ...
    async fn setup(
        ...
        let setup_outputs: Vec<Vec<_>> = {
            ...
            setup_outputs
                .into_iter()
                .map(|v: Vec<Secret<ArithmeticGmw<R>, Idx>>| {
...
impl DeltaSharing{
    pub fn arith_share<R: Ring>(&mut self, input: Vec<R>) -> (ShareVec<R>, Vec<R>)
    where
    ...
    pub fn arith_plain_delta_to_share<R>(&mut self, plaint_deltas: Vec<R>) -> ShareVec<R>
    where
        Standard: Distribution<R>
    {
        plaint_deltas
            .into_iter()
            .map(|plain_delta| Share {
                private: self.remote_joint_rng.gen(),
                public: plain_delta,
            })
            .collect()
    }

    pub fn arith_reconstruct<R: Ring>(a: ShareVec<R>, b: ShareVec<R>) -> Vec<R> {
        a.into_iter()
            .zip(b)
            .map(|(sh1, sh2)| {
                assert_eq!(
...
mod tests {
    ...
    async fn arithaby() {
        ...
        let setup0: AbySetupProvider<InsecureMTProvider<R>, R> =
            AbySetupProvider::new(0, InsecureMTProvider::default(), ch0.0, ch0.1);
        let setup1: AbySetupProvider<InsecureMTProvider<R>, R> =
            AbySetupProvider::new(1, InsecureMTProvider::default(), ch1.0, ch1.1);
        ...
        let res =
            DeltaSharing::arith_reconstruct(res1.into_scalar().unwrap(), res2.into_scalar().unwrap());
